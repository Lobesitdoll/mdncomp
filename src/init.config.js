...
function initConfig(options) {
  ...
  const _config = Object.assign({options: {}, formatter: {sepChar: char.sep}}, loadConfigFile());
  ...
  Object.keys(_config.options).forEach(key => {
    ...
    if (valid) {
      if ( valid.type(_config.options[ key ]) ) options[ key ] = valid.validate(_config.options[ key ]);
    }
    else {
  ...
  // sepChar
  if ( _config.formatter && typeof _config.formatter.sepChar === "string" ) {
    global.char.sep = _config.formatter.sepChar;
  }
  ...
  if (options.set) {
    ...
    if (!valid) {
      ...
      err(`${msg}?gValid keys:\n?w${utils.breakAnsiLine([...keys.keys()].sort().join(", "), options.maxChars)}?R`);
      process.exit()
    }
    ...
    if (value.length) {
      ...
      if (valid.type(_value)) {
        _config.options[key] = valid.validate(_value);
      }
      else {
    ...
    try {
      fs.writeFileSync(filePath, JSON.stringify(_config), "utf-8");
      process.exit()
    }
    catch(_err) {
