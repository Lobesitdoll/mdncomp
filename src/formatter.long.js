...
function compatToLong(mdnComp) {
  ...
  if (options.markdown && mdnComp.url.length) {
    ...
    out.addLine(" %0%1%2%3%4 %5", ANSI.yellow, mdnComp.prePath, ANSI.cyan, mdnComp.name, ANSI.reset, mdnComp.getStatus());
    out.addLine(" ", mdnComp.url ? mdnComp.url : "-", lf);
  }

  if (opts.showDesktop) {
    out.addLine(" %0DESKTOP:", ANSI.yellow);
    out.addLine(" %0Chrome    %1|%0 Edge      %1|%0 Firefox   %1|%0 IE        %1|%0 Opera     %1|%0 Safari%1", ANSI.green + ANSI.bright, ANSI.reset);
    out.addLine(" %0----------+-----------+-----------+-----------+-----------+-----------%1", ANSI.white + ANSI.dim, ANSI.reset);
  ...
  if (opts.showMobile) {
    ...
    out.addLine(" %0Webview/A %1|%0 Chrome/A  %1|%0 Edge/mob  %1|%0 Firefox/A %1|%0 Opera/A   %1|%0 Safari/iOS%1", ANSI.green + ANSI.bright, ANSI.reset);
    out.addLine(" %0----------+-----------+-----------+-----------+-----------+-----------%1", ANSI.white + ANSI.dim, ANSI.reset);
  ...
  function versions(list) {
    ...
    list.forEach(browserId => {
      ...
      if (browser) {
        ...
        if (browser.hasNotes()) {
          status += ANSI.white + (options.notes ? refs[ref] : "*") + ANSI.reset;
          notes.push(browser.getNotes(refs[ref]));
          ref++;
        }
        else if (browser.hasPrefix()) {
          status += ANSI.white + px8 + ANSI.reset;
        }
      ...
      out.add("%0" + status.centerAnsi(11) + "%1" + "|", status.indexOf(no) >= 0 ? ANSI.red : ANSI.cyan, ANSI.reset);
    });
